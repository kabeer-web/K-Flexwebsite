...
const Reviews = ({ productId, productName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL || "https://kflex-backend.vercel.app";
      await axios.post(`${apiUrl}/api/reviews`, { ...newReview, productId, productName });
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        <input type="text" placeholder="Your Name" value={newReview.userName}
        ...
        <textarea placeholder="Write your review..."
          ...
          required style={{ ...styles.input, gridColumn: "span 2", height: "100px" }} />
        <button type="submit" style={styles.button}>Submit Review ✍️</button>
      </form>

      <div style={{ ...styles.reviewList, maxHeight: showAll ? "none" : "500px", overflowY: showAll ? "visible" : "auto" }}>
        {Array.isArray(reviews) && reviews.length > 0 ? (
          reviews.map((review, index) => (
            <div key={index} style={styles.reviewCard}>
              <div style={styles.userRow}>
                <div style={styles.userIcon}>{review.userName?.charAt(0).toUpperCase()}</div>
                <div>
          ...
          <p style={{ textAlign: "center", color: "#6b7280" }}>No reviews found for this product.</p>
        )}
      </div>

      {Array.isArray(reviews) && reviews.length > 4 && (
        <div style={{ textAlign: "center", marginTop: "1rem" }}>
          <button onClick={() => setShowAll(!showAll)} style={styles.toggleButton}>
...
const styles = {
  wrapper: {
    ...
    padding: "2rem",
    borderRadius: "24px",
    background: "linear-gradient(to right bottom, rgba(255, 255, 255, 0.6), rgba(254, 181, 0, 0.2))",
